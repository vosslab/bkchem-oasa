#!/usr/bin/env python3
"""Build an HTML gallery of all sugar codes rendered as Haworth projection SVGs."""
...
def render_svg_file(
	...
	"""Render one sugar code combo and write SVG file with tight viewBox."""
...
def build_html(
	...
	"""Build complete HTML page for one ring type."""
	timestamp = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
	title = f"Sugar Codes - {ring_type.capitalize()}"
	# determine the other ring type for cross-link
	other_ring = "furanose" if ring_type == "pyranose" else "pyranose"
	sections_html = ""
	for category_name, cards in categories:
		# build card html for this category
		cards_html = ""
		for card in cards:
			...
			previews_html = ""
			if card["previews"]:
				for combo_label, svg_rel in card["previews"]:
					escaped_path = html.escape(svg_rel)
					previews_html += (
						f"<div class='preview-cell'>"
						f"<div class='combo-label'>{html.escape(combo_label)}</div>"
						f"<div class='svg-frame'>"
						f"<img class='preview' src='{escaped_path}' "
						f"alt='{code} {html.escape(combo_label)}'>"
						f"</div>"
						f"</div>\n"
					)
			else:
...
<body>
	<header>
		...
		<p class="stats">
			...
			{total_rendered} rendered SVG previews<br>
...
def render_all_svgs(
	...
	Returns:
		Dict keyed by ring_type, each value is a dict with keys:
			...
			total_rendered: int
	"""
	# initialize per-ring-type accumulators
	results = {}
	...
	for category_name, codes_dict in sugar_data.items():
		# per-ring-type card lists for this category
		...
		for code, name in codes_dict.items():
			for ring_type in RING_TYPES:
				...
				for anomeric in ANOMERIC_FORMS:
					...
					svg_filename = f"{code}_{ring_type}_{anomeric}.svg"
					svg_path = svg_dir / svg_filename
					try:
						render_svg_file(
							dom_ext, haworth_renderer, render_ops_mod,
							code, ring_type, anomeric, svg_path,
						)
						# relative path from output_dir to svg file
						svg_rel = os.path.relpath(svg_path, output_dir)
						previews.append((combo_label, svg_rel))
						results[ring_type]["total_rendered"] += 1
					except ValueError:
						# invalid ring/anomeric combo for this sugar
						pass
		...
		# append this category to each ring type
		for ring_type in RING_TYPES:
...
#============================================
def main() -> None:
	"""Render all sugar codes to SVG files and write separate HTML galleries."""
	repo_root = get_repo_root()
	output_dir = repo_root / "output_smoke"
	output_dir.mkdir(exist_ok=True)
	# SVG files go in a subdirectory
	svg_dir = output_dir / "sugar_codes_previews"
	svg_dir.mkdir(exist_ok=True)

	# load data and rendering modules
	sugar_data = load_sugar_codes(repo_root)
	dom_ext, haworth_renderer, render_ops_mod = load_oasa_modules(repo_root)

	# render all SVGs and collect per-ring-type data
	results = render_all_svgs(
	...
	html_filenames = {
		"pyranose": "sugar_codes_pyranose.html",
		"furanose": "sugar_codes_furanose.html",
	}
	...
	for ring_type in RING_TYPES:
		...
		print(f"Wrote {ring_type} gallery: {output_path}")
		print(f"  {data['total_sugars']} sugar codes | "
			f"{data['total_rendered']} rendered previews")
