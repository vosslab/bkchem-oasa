...
class BondCDMLMixin:
  ...
  def read_package(self, package):
    ...
    if type:
      ...
      self.type = "n"
      self.order = 1
    # sync composition shadow explicitly for clarity
    if hasattr(self, '_chem_bond'):
      self._chem_bond.type = self.type
      self._chem_bond.order = self.order
  ...
  def get_package(self, doc):
    ...
    # read chemistry properties from composition shadow when available
    bond_type = self._chem_bond.type if hasattr(self, '_chem_bond') else self.type
    bond_order = self._chem_bond.order if hasattr(self, '_chem_bond') else self.order
    dom_extensions.setAttributes(
      bnd,
      (
        ("type", "%s%d" % (bond_type, bond_order)),
        ("start", self.atom1.id),
        ("end", self.atom2.id),
        ("id", str(self.id)),
      ),
    ...
    if bond_order != 1:
      ...
      if bond_order == 2 and self.center is not None:
    ...
    if bond_type != "n" and bond_order != 1:
