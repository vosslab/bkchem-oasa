...
class vertex_common(object):
  """Properties and methods common for all vertices.
...
class drawable_chem_vertex(GraphVertexMixin,
                           meta_enabled,
                           area_colored,
                           point_drawable,
                           text_like,
                           child_with_paper,
                           vertex_common):
  """Common ancestor for all drawable chemistry vertices in BKChem.

  Combines GraphVertexMixin (graph connectivity) with GUI drawing parents.
  Chemistry properties (charge, valency, etc.) that were formerly inherited
  from oasa.chem_vertex are provided here so subclasses can override them.
  All methods are tuned for always shown texts. Atoms need to override some.
  """
  ...
  def __init__( self, standard=None, xy=(), molecule=None):
    ...
    GraphVertexMixin.__init__( self)

    # chemistry attrs formerly from oasa.chem_vertex
    self.charge = 0
    self.free_sites = 0
  ...
  def occupied_valency(self):
    ...
    for b in list(self._neighbors.keys()):
      # aromatic bond order 4 treated as 1
      order = b.order
      if order == 4:
        order = 1
      i += order
