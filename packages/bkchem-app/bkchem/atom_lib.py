...
class BkAtom(drawable_chem_vertex):
  ...
  def multiplicity(self, multiplicity):
    ...
    # sync composition attribute
    if hasattr(self, '_chem_atom'):
      self._chem_atom.multiplicity = multiplicity
  ...
  def occupied_valency(self):
    ...
    if self.charge:
      # deal with charge effects on valency
    ...
    if x > self.valency:
      # fallback to single bonds for aromatic to avoid over-counting
  ...
  def read_package( self, package):
    ...
    oasa.cdml_vertex_io.read_cdml_vertex_attributes(
      package, self,
      known_attrs=oasa.cdml_vertex_io.CDML_ATOM_KNOWN_ATTRS,
    )
