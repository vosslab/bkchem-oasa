...
def parse_version_registry_file(path: str) -> dict:
	"""Parse a canonical root VERSION assignment.

	Args:
		path: Root VERSION file path.

	Returns:
		Entry describing the canonical version registry.
	"""
...
def build_version_file_entry(path: str, version: str="", create: bool=True) -> dict:
	...
	Args:
		path: Canonical VERSION file path.
...
def release_metadata_entries(project_dir: str) -> list[dict]:
	"""Return root registry and package metadata entries for one release.

	Args:
		project_dir: A package directory or repository root.

	Returns:
		Registry and package metadata entries owned by the release.
	"""
...
def update_release_metadata(project_dir: str, version: str, apply: bool) -> list[dict]:
	"""Update one monorepo registry and every package release metadata source.

	Args:
		project_dir: A package directory or repository root.
		version: New shared release version.
		apply: Whether to write the planned updates.

	Returns:
		Per-file update results.
	"""
	entries = release_metadata_entries(project_dir)
	results = []
	for entry in entries:
		result = update_entry(entry, version, apply)
