...
- Increase `unit_03` bond/label spacing in
  ...
  `unit_03_bonds_nearby_strokes.json`.
...
- Update fixture runner defaults in
  [run_glyph_alignment_fixture_runner.py](../tools/run_glyph_alignment_fixture_runner.py)
...
- Add scaffold integrity checks in
  `test_glyph_alignment_fixture_scaffold.py`
...
- Tighten unit fixture tolerance gates in
  `unit_01_isolated_glyph_fit.json`
  and
  `unit_02_mixed_subscript_no_bonds.json`
...
- Reclassify [HAWORTH_OVERRIDES.md](HAWORTH_OVERRIDES.md) into
...
- Add a complete Haworth override catalog in
  [HAWORTH_OVERRIDES.md](HAWORTH_OVERRIDES.md) with line-anchored
...
- De-brittle hashed-coverage expectation in
  `test_haworth_renderer.py` to accept
...
- De-brittle Phase 4 guard tests while preserving contract coverage:
  - In `test_measure_glyph_bond_alignment.py`,
  ...
  - In `test_haworth_phase4_known_failures.py`,
...
- Update branch-angle/parity assertions in
  `test_haworth_renderer.py` for the
...
- Update Phase 4 closure status/checklist in
  [BOND_LABEL_GLYPH_CONTRACT_PLAN.md](archive/BOND_LABEL_GLYPH_CONTRACT_PLAN.md)
...
- Implement the manager-defined 11-target Phase 4 checklist and geometry gates:
  replace
  `test_haworth_phase4_known_failures.py`
...
- Add heuristic regression grep gates in
  `test_phase4_heuristic_grep_gate.py`
...
- Add minimal non-Haworth sentinel contract proofs in
  `test_phase4_non_haworth_sentinels.py`:
...
- Add Phase 3 policy regression coverage in
  [test_bond_length_policy.py](../packages/oasa/tests/test_bond_length_policy.py):
...
- Haworth explicit-H freeze active at tool callsites:
  ...
  [selftest_sheet.py](../tools/selftest_sheet.py) and
  [haworth_visual_check_pdf.py](../tools/haworth_visual_check_pdf.py),
  and add guard test
  [test_haworth_tool_show_hydrogens_freeze.py](../tests/test_haworth_tool_show_hydrogens_freeze.py)
...
- Add strict mode regression tests in
  `test_archive_matrix_summary_strict_modes.py`
...
- Re-export the shared connector target resolver in
  [haworth_renderer.py](../packages/oasa/oasa/haworth_renderer.py)
  and use it in
...
- Enforce architecture direction in
  [CODER_GUIDE_FEB_10.md](archive/CODER_GUIDE_FEB_10.md):
...
- Remove duplicated tool-local attach/strict logic from
  [archive_matrix_summary.py](../tools/archive_matrix_summary.py) and route
...
- Replace brittle "legacy fixed numbers" attach-box assertions and add
  ...
  [test_attach_targets.py](../packages/oasa/tests/test_attach_targets.py), and update
  Ribose C4-up tests in
  `test_haworth_renderer.py` to assert
  closed-center vertical attachment behavior.
- Align contract tests with runtime attach-target policy source of truth:
  `test_haworth_renderer_smoke.py`
  and `test_haworth_renderer.py` now use
...
- Update connector clipping regression in
  [test_connector_clipping.py](../packages/oasa/tests/test_connector_clipping.py) to use
...
- Add explicit two-carbon class parity coverage for the full D/L code sets
  ...
  `test_haworth_renderer.py`, including
...
- Update attach-target regression expectation in
  [test_attach_targets.py](../packages/oasa/tests/test_attach_targets.py) to assert the
...
- Add active plan
  [HAWORTH_EXPLICIT_HYDROGEN_PLAN.md](active_plans/HAWORTH_EXPLICIT_HYDROGEN_PLAN.md)
...
- Extend
  ...
  by design).
- Update
  [test_attach_targets.py](../packages/oasa/tests/test_attach_targets.py)
...
- Close recovery gates from
  [COMPLETE_BOND_LABEL_PLAN.md](archive/COMPLETE_BOND_LABEL_PLAN.md)
...
- Refresh architecture documentation in
  `CODE_ARCHITECTURE.md` to describe OASA-based
...
- Add Phase A unit coverage in
  [test_attach_targets.py](../packages/oasa/tests/test_attach_targets.py) for target
  ...
  [test_connector_clipping.py](../packages/oasa/tests/test_connector_clipping.py) with an
...
- Add direct unit coverage for
  ...
  [test_attach_targets.py](../packages/oasa/tests/test_attach_targets.py), including
...
- Decompose `bond.py` into focused mixins without behavior change:
  add
  [bond_drawing.py](../packages/bkchem-app/bkchem/bond_drawing.py),
...
- Tighten quick Haworth regression guards:
  in `test_haworth_renderer.py`, replace
...
- Fix stale parity plan reference in
  `CHANGELOG.md` to point at
  [RENDER_LAYOUT_PARITY_SPEC.md](archive/RENDER_LAYOUT_PARITY_SPEC.md).
- Expand
  [COMPLETE_BOND_LABEL_PLAN.md](archive/COMPLETE_BOND_LABEL_PLAN.md)
...
- Restore rounded side wedges for Haworth ring edges adjacent to the front edge
  in [renderer.py](../packages/oasa/oasa/haworth/renderer.py):
...
- Implement directional token-edge label attachment as follow-on clipping
  behavior in `render_geometry.py`:
...
- Add directional attachment regressions in
  [test_label_bbox.py](../packages/oasa/tests/test_label_bbox.py),
  `test_haworth_renderer.py`, and keep
  overlap/parity gates green in
  [test_connector_clipping.py](../packages/oasa/tests/test_connector_clipping.py),
  `test_haworth_renderer_smoke.py`,
  and `test_phase_c_render_pipeline.py`.
- Refine true hashed-branch semantics for furanose two-carbon tails in
  ...
  checks in `test_haworth_renderer.py`
...
- Tighten Haworth hydroxyl connector acceptance to block own-label penetration:
  update `test_haworth_renderer_smoke.py`
  ...
  regressions in `test_haworth_renderer.py`
...
- Refine furanose two-carbon tail rendering to match reference directionality
  ...
  in `test_haworth_renderer.py` and keep
...
- Complete render-layout parity implementation from
  ...
  `test_render_layout_parity.py` for
...
- Add [RENDER_LAYOUT_PARITY_SPEC.md](archive/RENDER_LAYOUT_PARITY_SPEC.md)
...
- Fix Haworth connector regression after shared-clipping integration by splitting
  ...
  `test_haworth_renderer.py` and document
  the split policy in
...
- Clean stale backlog/status docs by removing obsolete `bkchem/oasa` validation
  from [TODO_REPO.md](TODO_REPO.md), updating RDKit/Open Babel
  candidate wiring in [TODO_CODE.md](TODO_CODE.md) from legacy plugin
...
- Remove `TODO.md` and move its remaining backlog-hygiene
  action into [TODO_REPO.md](TODO_REPO.md) so TODO tracking lives
...
- Clarify CDML namespace usage and add explicit documentation-pointer metadata:
  ...
  [CDML_FORMAT_SPEC.md](CDML_FORMAT_SPEC.md), and add regression
  coverage in [test_codec_registry.py](../packages/oasa/tests/test_codec_registry.py) and
  `test_bkchem_cdml_roundtrip.py`.
- Fix CD-SVG CDML forwarding in
  [cdsvg.py](../packages/oasa/oasa/codecs/cdsvg.py) by
  ...
  a regression test in [test_codec_registry.py](../packages/oasa/tests/test_codec_registry.py)
  that validates forwarded `version`/`namespace` values appear in embedded
  CDML.
- Implement Phase 1 of
  [BOND_LABEL_ATTACHMENT_IMPROVEMENT_PLAN.md](archive/BOND_LABEL_ATTACHMENT_IMPROVEMENT_PLAN.md):
...
- Tighten Phase 1 validation by making
  ...
  `render_geometry.py`,
  with regression coverage in [test_label_bbox.py](../packages/oasa/tests/test_label_bbox.py).
- Implement Phase 2 connector clipping integration from
  ...
  [test_connector_clipping.py](../packages/oasa/tests/test_connector_clipping.py) plus
  `test_haworth_renderer.py`.
- Finalize Phase 2 hardening for
  ...
  [test_connector_clipping.py](../packages/oasa/tests/test_connector_clipping.py) and
  [test_label_bbox.py](../packages/oasa/tests/test_label_bbox.py), add a focused
  tokenization fixture matrix (`OAc`, `NHCH3`, `SO3H`, `PPh3`,
  `CH(OH)CH2OH`) in [test_label_bbox.py](../packages/oasa/tests/test_label_bbox.py), and
...
- Tighten attachment and coordinate-system wording in
  [BOND_LABEL_ATTACHMENT_IMPROVEMENT_PLAN.md](archive/BOND_LABEL_ATTACHMENT_IMPROVEMENT_PLAN.md)
  and [CDML_FORMAT_SPEC.md](CDML_FORMAT_SPEC.md): make missing
...
- Complete Phase C renderer migration from
  [PURE_OASA_BACKEND_REFACTOR.md](archive/PURE_OASA_BACKEND_REFACTOR.md):
  add OASA registry-backed render codecs (`svg`, `pdf`, `png`, `ps`) plus a
  new CD-SVG codec in
  [codec_registry.py](../packages/oasa/oasa/codec_registry.py),
  [render.py](../packages/oasa/oasa/codecs/render.py),
  and [cdsvg.py](../packages/oasa/oasa/codecs/cdsvg.py);
...
- Add Phase C regression coverage in
  ...
  [test_codec_registry.py](../packages/oasa/tests/test_codec_registry.py) and
  `test_codec_registry_bkchem_plugins.py`,
  and align plugin smoke inventory in
  `bkchem_plugin_smoke.py`.
- Phase C cleanup follow-ups: consolidate duplicated molecule merge helpers into
  [molecule_utils.py](../packages/oasa/oasa/molecule_utils.py)
  and route both
  [render_out.py](../packages/oasa/oasa/render_out.py) and
  `oasa_bridge.py`
  through that shared utility; remove redundant SVG root append in
  [render_out.py](../packages/oasa/oasa/render_out.py); avoid
  ...
  [cdsvg.py](../packages/oasa/oasa/codecs/cdsvg.py).
...
- Remove dead legacy CML export code paths in
  [cml.py](../packages/oasa/oasa/codecs/cml.py) and
  [cml2.py](../packages/oasa/oasa/codecs/cml2.py) so
...
- Complete Phase B audit/retention work for
  ...
  [PHASE_B_AUDIT.md](archive/PHASE_B_AUDIT.md),
  mark Phase B status/done gates complete in
  [PURE_OASA_BACKEND_REFACTOR.md](archive/PURE_OASA_BACKEND_REFACTOR.md),
  and add follow-up backlog items in [TODO_CODE.md](TODO_CODE.md).
...
- Add Phase B guardrail coverage in
  `test_phase_b_option_policy.py`
  and `test_phase_b_gtml_roundtrip.py`,
  and extend
  `test_codec_registry_bkchem_plugins.py`
...
- Implement Phase A plumbing from
  ...
  [cml.py](../packages/oasa/oasa/codecs/cml.py) and
  [cml2.py](../packages/oasa/oasa/codecs/cml2.py),
  add BKChem registry-driven format plumbing via
  [format_loader.py](../packages/bkchem-app/bkchem/format_loader.py)
  and
  [format_menus.yaml](../packages/bkchem-app/bkchem/format_menus.yaml),
  route menu handling through the loader in
  [main.py](../packages/bkchem-app/bkchem/main.py), remove
...
- Replace deleted-plugin tests with loader/bridge coverage in
  ...
  [test_codec_registry.py](../packages/oasa/tests/test_codec_registry.py), extend
...
- Add [PURE_OASA_BACKEND_REFACTOR.md](archive/PURE_OASA_BACKEND_REFACTOR.md)
...
- Trim [SUGAR_CODE_SPEC.md](SUGAR_CODE_SPEC.md) to keep only
...
- Correct D-ketohexose name mapping by swapping `MKRRDM`/`MKLLDM` assignments
  ...
  [sugar_codes.yaml](../packages/oasa/oasa_data/sugar_codes.yaml),
  [SUGAR_CODE_SPEC.md](SUGAR_CODE_SPEC.md), and
  [HAWORTH_IMPLEMENTATION_PLAN_attempt2.md](archive/HAWORTH_IMPLEMENTATION_PLAN_attempt2.md),
  and align fixture pairings in
  `neurotiker_archive_mapping.py`
  and `archive_ground_truth.py`.
- Remove unused legacy `get_structure_hash()` from
  `molecule.py` to eliminate
  Python 2-era `sha` import-policy issues in Python 3.
- Add a backlog item in [TODO_CODE.md](TODO_CODE.md) to implement a
  hash-based verification layer for future CDML files.
- Fix invalid UTF-8 bytes in
  [HAWORTH_CODE_ORGANIZATION_PLAN.md](archive/HAWORTH_CODE_ORGANIZATION_PLAN.md)
...
- Remove SOAP support references by deleting the legacy SOAP server startup
  block from [main.py](../packages/bkchem-app/bkchem/main.py)
  and dropping `SOAPpy` from `pip_requirements.txt`.
...
- Complete codec-registry Phase 3 backend routing by moving CML/CML2/CDXML
  ...
  ([cml.py](../packages/oasa/oasa/codecs/cml.py),
  [cml2.py](../packages/oasa/oasa/codecs/cml2.py), and
  [cdxml.py](../packages/oasa/oasa/codecs/cdxml.py)),
  registering them in
  [codec_registry.py](../packages/oasa/oasa/codec_registry.py),
  extending `oasa_bridge.py`
...
- Update import policy sources in
  [test_import_requirements.py](../tests/test_import_requirements.py) to
...
- Move Haworth modules into dedicated
  ...
  ([haworth_renderer.py](../packages/oasa/oasa/haworth_renderer.py),
  [haworth_spec.py](../packages/oasa/oasa/haworth_spec.py))
  keep all existing `import oasa.haworth_renderer` / `import oasa.haworth_spec`
  code working without changes.
- Add [sugar_code_names.py](../packages/oasa/oasa/sugar_code_names.py):
...
- Add [SUGAR_CODE_GUIDE.md](SUGAR_CODE_GUIDE.md) to document sugar
...
- Align generated preview oxygen-mask fill with summary frame background in
  [archive_matrix_summary.py](../tools/archive_matrix_summary.py) by
...
- Fix ALRDM top-label regression in
  [haworth_renderer.py](../packages/oasa/oasa/haworth_renderer.py)
  and [render_ops.py](../packages/oasa/oasa/render_ops.py):
...
- Improve chemical subscript readability in
  [render_ops.py](../packages/oasa/oasa/render_ops.py) by
...
- Refine furanose interior pair spacing rule in
  ...
  Update coverage in `test_haworth_renderer.py`
...
- Fix left-end 2-carbon exocyclic chain direction labeling for furanose
  ...
  coverage in `test_haworth_renderer.py`
...
- Tune exocyclic `CH<sub>2</sub>OH` connector anchoring and internal-label polish:
  in [haworth_renderer.py](../packages/oasa/oasa/haworth_renderer.py),
  ...
  text size. In [render_ops.py](../packages/oasa/oasa/render_ops.py),
...
- Refine internal hydroxyl layout in
  ...
  `test_haworth_renderer.py` for ring-bond
...
- Extend furanose top-side `up` connector lengths in
  ...
  in `test_haworth_renderer.py` for
...
- Fix generated-preview scaling inconsistency in
  [archive_matrix_summary.py](../tools/archive_matrix_summary.py) by normalizing
...
- Add two-pass hydroxyl label layout in
  ...
  `test_haworth_renderer.py` for ALDM
  furanose-alpha spacing and direct candidate-slot selection behavior.
- Add CLI argument parsing to
  [archive_matrix_summary.py](../tools/archive_matrix_summary.py) with
...
- Calibrate furanose ring geometry from NEUROtiker archive references by adding
  ...
  `haworth.py` and update
  regression expectations in
  `test_haworth_renderer.py`.
- Refine furanose ring geometry in `haworth.py`
...
- Improve hydroxyl connector/text separation in
  ...
  oxygen glyph region.
- Harden [archive_matrix_summary.py](../tools/archive_matrix_summary.py) XML
  ...
  checks.
- Update [archive_matrix_summary.py](../tools/archive_matrix_summary.py) so
...
- Adjust hydroxyl label x-offset geometry in
  ...
  `test_haworth_renderer.py` covering each
  anchor direction.
- Improve summary-page centering/scaling in
  [archive_matrix_summary.py](../tools/archive_matrix_summary.py) by generating
...
- Add [archive_matrix_summary.py](../tools/archive_matrix_summary.py) to build
...
- Start Phase 6 sugar-code-to-SMILES implementation with a bootstrap converter
  in [sugar_code_smiles.py](../packages/oasa/oasa/sugar_code_smiles.py),
  export it from `__init__.py`,
  and add unit coverage in
  [test_sugar_code_smiles.py](../packages/oasa/tests/test_sugar_code_smiles.py) for
...
- Update [HAWORTH_IMPLEMENTATION_PLAN_attempt2.md](archive/HAWORTH_IMPLEMENTATION_PLAN_attempt2.md)
...
- Update `AGENTS.md` environment instructions to require
...
- Add archive-wide Haworth validation coverage:
  `test_haworth_spec.py` now parametrizes
...
- Extend renderer regression tests in
  `test_haworth_renderer.py` for oxygen-edge
...
- Expand Haworth selftest visual vignettes in
  [selftest_sheet.py](../tools/selftest_sheet.py) by adding a third row with
...
- Add a focused Wikimedia downloader script at
  [fetch_neurotiker_haworth_archives.py](../tools/fetch_neurotiker_haworth_archives.py)
...
- Implement Phase 4 Haworth selftest integration in
  [selftest_sheet.py](../tools/selftest_sheet.py): route
...
- Add Phase 4 integration test coverage in
  `test_selftest_haworth_builders.py`
...
- Add Phase 3 unit coverage in
  `test_haworth_renderer.py` (31 tests)
...
- Implement Phase 2 Haworth spec generation in
  [haworth_spec.py](../packages/oasa/oasa/haworth_spec.py) with
...
- Add Phase 2 unit/smoke coverage in
  `test_haworth_spec.py` and
  [test_haworth_spec_smoke.py](../packages/oasa/tests/smoke/test_haworth_spec_smoke.py),
...
- Add a compact standard sanity-matrix test in
  `test_haworth_spec.py` for quick human-check
...
- Fix test/lint regressions after fixture cleanup:
  [test_cdml_versioning.py](../packages/bkchem-app/tests/test_cdml_versioning.py) now skips the
  legacy fixture check when `tests/fixtures/cdml/legacy_v0.11.cdml` is absent;
  [check_translation.py](../tools/check_translation.py) now uses consistent
  tab indentation and a `main()` guard to satisfy shebang/indentation checks;
  and [selftest_sheet.py](../tools/selftest_sheet.py) drops two pyflakes
...
- Simplify reference-output policy to Haworth-only artifacts: update
  ...
  [render_reference_outputs.py](../tools/render_reference_outputs.py), and
  [REFERENCE_OUTPUTS.md](REFERENCE_OUTPUTS.md) to stop expecting or
  generating `wavy_glucose_reference.svg/png`.
- Update [test_cdml_fixture_loads.py](../packages/oasa/tests/test_cdml_fixture_loads.py) to
...
- Implement Phase 1 sugar-code parser in
  [sugar_code.py](../packages/oasa/oasa/sugar_code.py) with
...
- Add Phase 1 parser tests in
  [test_sugar_code.py](../packages/oasa/tests/test_sugar_code.py) (28 unit cases) and
  [test_sugar_code_smoke.py](../packages/oasa/tests/smoke/test_sugar_code_smoke.py)
  using `smoke_sugar_codes.txt`
...
- Export `atom_colors` and `dom_extensions` from
  ...
  `AttributeError` in `test_fischer_explicit_h.py`.
...
- Remove time estimates from menu refactor phases in
  [MENU_REFACTOR_ANALYSIS.md](archive/MENU_REFACTOR_ANALYSIS.md),
  [MODULAR_MENU_ARCHITECTURE.md](archive/MODULAR_MENU_ARCHITECTURE.md), and
  [MENU_REFACTOR_SUMMARY.md](archive/MENU_REFACTOR_SUMMARY.md).
- Add Phase 0 baseline performance measurement requirement to menu refactor
  plans in
  [BKCHEM_GUI_MENU_REFACTOR.md](archive/BKCHEM_GUI_MENU_REFACTOR.md),
  [MENU_REFACTOR_ANALYSIS.md](archive/MENU_REFACTOR_ANALYSIS.md), and
  [MENU_REFACTOR_SUMMARY.md](archive/MENU_REFACTOR_SUMMARY.md): measure
...
- Tighten digit/footnote rule consistency in
  [SUGAR_CODE_SPEC.md](SUGAR_CODE_SPEC.md): change minimum-length
...
- Tighten canonical carbon-state consistency in
  [SUGAR_CODE_SPEC.md](SUGAR_CODE_SPEC.md): update pyruvate to
...
- Tighten backbone-count wording in [SUGAR_CODE_SPEC.md](SUGAR_CODE_SPEC.md)
...
- Clarify mode semantics and terminal naming in
  [SUGAR_CODE_SPEC.md](SUGAR_CODE_SPEC.md): rename `TERMINAL` to
...
- Replace provisional branch-word notation in
  [SUGAR_CODE_SPEC.md](SUGAR_CODE_SPEC.md) with explicit substituent
...
- Align parser-plan wording in
  ...
  validation language.
- Convert pathway codebook notation in [SUGAR_CODE_SPEC.md](SUGAR_CODE_SPEC.md)
...
- Revise backbone-length semantics in [SUGAR_CODE_SPEC.md](SUGAR_CODE_SPEC.md)
  and [HAWORTH_IMPLEMENTATION_PLAN_attempt2.md](archive/HAWORTH_IMPLEMENTATION_PLAN_attempt2.md):
...
- Normalize pyruvate canonical token in [SUGAR_CODE_SPEC.md](SUGAR_CODE_SPEC.md)
...
- Expand pathway-profile completeness in
  ...
  Phase 0 sign-off.
- Update [SUGAR_CODE_SPEC.md](SUGAR_CODE_SPEC.md) to keep pathway
...
- Refine `HaworthSpec` contract in
  [HAWORTH_IMPLEMENTATION_PLAN_attempt2.md](archive/HAWORTH_IMPLEMENTATION_PLAN_attempt2.md)
  and [SUGAR_CODE_SPEC.md](SUGAR_CODE_SPEC.md): remove `config` from
...
- Expand [SUGAR_CODE_SPEC.md](SUGAR_CODE_SPEC.md) and
  [HAWORTH_IMPLEMENTATION_PLAN_attempt2.md](archive/HAWORTH_IMPLEMENTATION_PLAN_attempt2.md)
...
- Update parser schema details in
  [HAWORTH_IMPLEMENTATION_PLAN_attempt2.md](archive/HAWORTH_IMPLEMENTATION_PLAN_attempt2.md)
  and [SUGAR_CODE_SPEC.md](SUGAR_CODE_SPEC.md) to split former `raw`
...
- Update parser schema language in
  [HAWORTH_IMPLEMENTATION_PLAN_attempt2.md](archive/HAWORTH_IMPLEMENTATION_PLAN_attempt2.md)
  and [SUGAR_CODE_SPEC.md](SUGAR_CODE_SPEC.md): internal `prefix` is now
  documented as normalized kind (`ALDO`/`KETO`/`3-KETO`) while literal prefix tokens
  remain in raw input text.
- Add an explicit uronic-acid note to [SUGAR_CODE_SPEC.md](SUGAR_CODE_SPEC.md)
...
- Add explicit invalid-prefix rationale in
  [SUGAR_CODE_SPEC.md](SUGAR_CODE_SPEC.md) and parser requirements in
  [HAWORTH_IMPLEMENTATION_PLAN_attempt2.md](archive/HAWORTH_IMPLEMENTATION_PLAN_attempt2.md):
...
- Improve visual clarity in the PREFIX subsection of
  [SUGAR_CODE_SPEC.md](SUGAR_CODE_SPEC.md) by adding aligned star-fill
...
- Update [SUGAR_CODE_SPEC.md](SUGAR_CODE_SPEC.md) to add a Haworth ring
...
- Expand Phase 0 documentation in
  [HAWORTH_IMPLEMENTATION_PLAN_attempt2.md](archive/HAWORTH_IMPLEMENTATION_PLAN_attempt2.md)
  and [SUGAR_CODE_SPEC.md](SUGAR_CODE_SPEC.md) with explicit
...
- Update all tools to use `git rev-parse --show-toplevel` for repo root detection
  per [REPO_STYLE.md](REPO_STYLE.md) guidance.
...
- Add `beta-D-fructofuranose` SMILES to [biomolecule_smiles.yaml](../packages/oasa/oasa_data/biomolecule_smiles.yaml).
...
- Add [RELEASE_DISTRIBUTION.md](active_plans/RELEASE_DISTRIBUTION.md) to the
  refactor progress plan as a not-started automated installer tooling item.
- Update [RELEASE_DISTRIBUTION.md](active_plans/RELEASE_DISTRIBUTION.md) to add
...
- Fix non-ASCII characters in [MODULAR_MENU_ARCHITECTURE.md](archive/MODULAR_MENU_ARCHITECTURE.md)
...
- Add platform abstraction layer to [BKCHEM_GUI_MENU_REFACTOR.md](archive/BKCHEM_GUI_MENU_REFACTOR.md)
...
- Add performance monitoring section to [BKCHEM_GUI_MENU_REFACTOR.md](archive/BKCHEM_GUI_MENU_REFACTOR.md)
...
- Add [MODULAR_MENU_ARCHITECTURE.md](archive/MODULAR_MENU_ARCHITECTURE.md)
...
- Add [MENU_REFACTOR_ANALYSIS.md](archive/MENU_REFACTOR_ANALYSIS.md) with
...
- Add "GPL v2 Code Coverage Assessment Plan" section to
  [LICENSE_MIGRATION.md](active_plans/LICENSE_MIGRATION.md) with comprehensive
...
- Move SVG vertex rendering to ops: add `build_vertex_ops` helpers in
  `render_geometry.py`,
  switch [svg_out.py](../packages/oasa/oasa/svg_out.py) and
  selftest molecule ops to use them.
- Add [BKCHEM_GUI_MENU_REFACTOR.md](archive/BKCHEM_GUI_MENU_REFACTOR.md)
  documenting current menu construction and a refactor plan.
- Comprehensively rewrite [BKCHEM_GUI_MENU_REFACTOR.md](archive/BKCHEM_GUI_MENU_REFACTOR.md)
...
- Add [BKCHEM_GUI_CODE_REVIEW.md](archive/BKCHEM_GUI_CODE_REVIEW.md) with
  a GUI code quality review and prioritized improvement areas.
- Expand [BKCHEM_GUI_CODE_REVIEW.md](archive/BKCHEM_GUI_CODE_REVIEW.md) with
...
- Add [BACKEND_CODE_REVIEW.md](archive/BACKEND_CODE_REVIEW.md) documenting
...
- Update [SELFTEST_PAGE_SPEC.md](SELFTEST_PAGE_SPEC.md) with "Vignette
...
- Refresh [BKCHEM_FORMAT_SPEC.md](archive/BKCHEM_FORMAT_SPEC.md),
  `CUSTOM_PLUGINS.md`,
  [TODO_CODE.md](TODO_CODE.md),
  [CDML_ARCHITECTURE_PLAN.md](archive/CDML_ARCHITECTURE_PLAN.md), and
...
- Replace test-local repo root and fixtures path lookups with
  `conftest.py` helpers, removing `os.path.dirname(__file__)`
...
- Add the selftest motto ("Do less, but show more. Let the backend handle the
  complexities.") to [SELFTEST_PAGE_SPEC.md](SELFTEST_PAGE_SPEC.md)
  and [RENDERER_CAPABILITIES_SHEET_PLAN.md](active_plans/RENDERER_CAPABILITIES_SHEET_PLAN.md).
- Resolve repo root in `run_smoke.sh` from the script
...
- Replace non-ASCII box drawing and checkmark glyphs in
  [BKCHEM_GUI_MENU_REFACTOR.md](archive/BKCHEM_GUI_MENU_REFACTOR.md) with
  ASCII equivalents to satisfy compliance checks.
- Add review comments to [BKCHEM_GUI_MENU_REFACTOR.md](archive/BKCHEM_GUI_MENU_REFACTOR.md)
  covering translation extraction, menu label stability, Pmw component lookup,
  and plugin insertion compatibility.
- Update [BKCHEM_GUI_MENU_REFACTOR.md](archive/BKCHEM_GUI_MENU_REFACTOR.md)
...
- Add review comments to [LICENSE_MIGRATION.md](active_plans/LICENSE_MIGRATION.md)
...
- Clarify in [LICENSE_MIGRATION.md](active_plans/LICENSE_MIGRATION.md) that mixed
...
- Add preferred one-command usage examples for the GPL coverage assessment
  script (summary, full report, CSV) to [LICENSE_MIGRATION.md](active_plans/LICENSE_MIGRATION.md).
- Add [assess_gpl_coverage.py](../tools/assess_gpl_coverage.py) for
  reporting-only GPL/LGPL coverage metrics and update
  [LICENSE_MIGRATION.md](active_plans/LICENSE_MIGRATION.md) to reference the
  tools path.
- Fix date handling in [assess_gpl_coverage.py](../tools/assess_gpl_coverage.py)
  by comparing date values to avoid timezone-aware vs naive datetime errors.
- Add an ASCII progress bar to [assess_gpl_coverage.py](../tools/assess_gpl_coverage.py)
  to show scan status while building coverage records.
- Fix GPL/LGPL classification in [assess_gpl_coverage.py](../tools/assess_gpl_coverage.py)
...
- Prefer SPDX headers and legacy license text detection when classifying files
  in [assess_gpl_coverage.py](../tools/assess_gpl_coverage.py), and record the
  classification source in CSV output.
- Clamp GPL time-percentage calculations in
  [assess_gpl_coverage.py](../tools/assess_gpl_coverage.py) when the cutoff
  date predates the first commit to avoid negative values.
- Update [LICENSE_MIGRATION.md](active_plans/LICENSE_MIGRATION.md) to require
...
- Update [assess_gpl_coverage.py](../tools/assess_gpl_coverage.py) to
...
- Replace non-ASCII checkmark glyphs in
  [MODULAR_MENU_ARCHITECTURE.md](archive/MODULAR_MENU_ARCHITECTURE.md) with
  ASCII equivalents for compliance.
- Add per-file spot-check samples (git blame lines around the cutoff) to
  [assess_gpl_coverage.py](../tools/assess_gpl_coverage.py) output when
  using `--file`.
...
- Clarify GPLv2 vs LGPLv3 labeling in
  [assess_gpl_coverage.py](../tools/assess_gpl_coverage.py) output for
  summaries and per-file reports.
- Silence SPDX "missing" noise in [assess_gpl_coverage.py](../tools/assess_gpl_coverage.py)
...
## 2026-02-01
- Update [SELFTEST_PAGE_SPEC.md](SELFTEST_PAGE_SPEC.md) to establish
...
- Mark CDML architecture plan phases 1 and 2 complete in
  [CDML_ARCHITECTURE_PLAN.md](archive/CDML_ARCHITECTURE_PLAN.md).
...
- Add the codec registry plan in
  [CODEC_REGISTRY_PLAN.md](archive/CODEC_REGISTRY_PLAN.md).
...
- Add capabilities sheet layout specification in
  [SELFTEST_PAGE_SPEC.md](SELFTEST_PAGE_SPEC.md) documenting the
  measure-first layout system and vignette organization.
- Add CDML architecture plan in
  [CDML_ARCHITECTURE_PLAN.md](archive/CDML_ARCHITECTURE_PLAN.md) for future
  BKChem/OASA separation with layer responsibilities and phased integration.
- Add [LICENSE_MIGRATION.md](active_plans/LICENSE_MIGRATION.md) with a
...
- Add [RENDER_BACKEND_UNIFICATION.md](archive/RENDER_BACKEND_UNIFICATION.md)
  to plan the shared render-ops backend for SVG and Cairo.
- Refine [RENDER_BACKEND_UNIFICATION.md](archive/RENDER_BACKEND_UNIFICATION.md)
  with a shared context-provider rule and ops-JSON acceptance criteria.
- Add invariants and scope boundaries to
  [RENDER_BACKEND_UNIFICATION.md](archive/RENDER_BACKEND_UNIFICATION.md) to
...
- Update [ROUNDED_WEDGES_PLAN.md](archive/ROUNDED_WEDGES_PLAN.md) to describe
...
- Add biomolecule template insertion smoke coverage in
  ...
  `tests/test_template_catalog.py`.
- Add [REFERENCE_OUTPUTS.md](REFERENCE_OUTPUTS.md) and a
...
- Refresh `CODE_ARCHITECTURE.md` and
  `FILE_STRUCTURE.md` with updated component and
...
- Add [ROUNDED_WEDGES_PLAN.md](archive/ROUNDED_WEDGES_PLAN.md) with a
