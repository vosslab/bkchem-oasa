...
- [main.py](../../packages/bkchem-app/bkchem/main.py) - main application class, menu system, mode toolbar
- [paper.py](../../packages/bkchem-app/bkchem/paper.py) - drawing canvas (chem_paper class), event bindings
- `modes.py` - mode classes, key sequence handling
- `context_menu.py` - right-click context menus
- [interactors.py](../../packages/bkchem-app/bkchem/interactors.py) - user interaction dialogs
- [singleton_store.py](../../packages/bkchem-app/bkchem/singleton_store.py) - global singleton pattern
- `plugin_support.py` - plugin system
...
- High: Batch mode runs arbitrary script files with `exec` and no path
  ...
  [main.py](../../packages/bkchem-app/bkchem/main.py) lines
  1435-1446.
- High: Plugin execution loads and `exec`s Python scripts directly in the UI
  ...
  event loop. See `plugin_support.py`
  lines 84-110.
- Medium: Global singleton state (`Store.app`, `Screen.dpi`, and other Store
  ...
  between UI and model. See [main.py](../../packages/bkchem-app/bkchem/main.py)
  lines 69-87 and [paper.py](../../packages/bkchem-app/bkchem/paper.py)
  lines 137-165.
- Medium: Input handling is platform specific and incomplete. Mouse wheel
  ...
  scroll behavior across platforms. See [paper.py](../../packages/bkchem-app/bkchem/paper.py)
  lines 158-168.
...
The menu system (see [main.py](../../packages/bkchem-app/bkchem/main.py) lines 178-289) uses a flat tuple-based template with an implicit schema:
...
The mode system (see `modes.py`) implements a state machine where each mode handles mouse and keyboard events differently. Key observations:

- **Mode registration**: Modes are registered in `init_modes()` at [main.py](../../packages/bkchem-app/bkchem/main.py) line 456, with a separate ordering list `modes_sort` (line 476) that controls toolbar button order
...
The key sequence system (see `modes.py` lines 113-168) implements Emacs-style key chords:
...
The context menu system (see `context_menu.py`) dynamically builds menus based on the current selection:
...
The canvas (chem_paper class) sets up event bindings in `set_bindings()` at [paper.py](../../packages/bkchem-app/bkchem/paper.py) lines 137-163:
...
Singleton managers are initialized in `init_singletons()` at [main.py](../../packages/bkchem-app/bkchem/main.py) lines 407-433:
...
All are stored in the global `Store` class (see [singleton_store.py](../../packages/bkchem-app/bkchem/singleton_store.py)), making them accessible anywhere but hard to test or replace.
...
4. **Menu system refactor**: See detailed plan in [BKCHEM_GUI_MENU_REFACTOR.md](BKCHEM_GUI_MENU_REFACTOR.md). Create an action registry with declarative menu definitions.
